...
RECOMMENDED_ACTIONS = {
    "safe": [
        "Verify the domain before sharing personal data.",
        "Use strong, unique passwords.",
        "Stay cautious with links from unknown sources.",
    ],
    "suspicious": [
        "Do not log in or enter credentials on this site.",
        "Do not share OTP codes or passwords.",
        "Report if you already entered data.",
        "Change passwords if you may have been compromised.",
    ],
    "scam": [
        "Do not visit this URL.",
        "Do not share any personal or financial information.",
        "Report to authorities if you were targeted.",
        "Change passwords immediately if you entered them.",
    ],
}

WHAT_TO_DO_NOW = {
    "safe": "Verify the domain; be cautious with personal data; use strong passwords.",
    "suspicious": "Do not log in; do not share OTP or passwords; report if you already entered data; change passwords if compromised.",
    "scam": "Do not visit; do not share any information; report to authorities; change passwords if you entered them.",
}
...
def generate_scan_pdf(scan: Scan) -> bytes:
    ...
    verdict_badge = verdict.upper()
    verdict_color = {"safe": "green", "suspicious": "orange", "scam": "red"}.get(verdict, "black")
    ...
    elements.append(Paragraph(f"<b>Threat Level:</b> {scan.threat_level or 'N/A'} | <b>Score:</b> {scan.score}/100", meta_style))
    elements.append(Spacer(1, 0.05 * inch))
    elements.append(Paragraph(PLAIN_MEANING.get(verdict, PLAIN_MEANING["safe"]), meta_style))
    elements.append(Spacer(1, 0.08 * inch))
    elements.append(Paragraph("<b>Recommended actions:</b>", meta_style))
    for action in RECOMMENDED_ACTIONS.get(verdict, RECOMMENDED_ACTIONS["safe"]):
        elements.append(Paragraph(f"&bull; {action}", small_style))
    elements.append(Spacer(1, 0.2 * inch))
    ...
    elements.append(Paragraph("<b>What You Should Do Now</b>", meta_style))
    elements.append(Spacer(1, 0.05 * inch))
    elements.append(Paragraph(WHAT_TO_DO_NOW.get(verdict, WHAT_TO_DO_NOW["safe"]), meta_style))
    elements.append(Spacer(1, 0.2 * inch))
