...
def send_email_otp(body: SendEmailOtpRequest, db: Session = Depends(get_db)):
    ...
    if not is_email_otp_configured():
        raise HTTPException(
            status_code=503,
            detail="Email OTP not configured. Add SMTP_HOST, SMTP_USER, SMTP_PASSWORD to backend/.env",
        )
    try:
    ...
    if not send_otp_email(email, code):
        raise HTTPException(503, "Failed to send email. Check SMTP settings.")
...
def verify_2fa(body: Verify2FARequest, request: Request, db: Session = Depends(get_db)):
    ...
    # Accept either TOTP (authenticator app) or email OTP
    totp_ok = False
    email_otp_ok = verify_email_otp(body.temp_token, body.code)

    if not email_otp_ok:
        sec = db.query(UserSecurity).filter(UserSecurity.user_id == admin.id).first()
        if sec and sec.totp_secret:
...
def change_password(
    ...
    return {"ok": True, "message": "Password updated", "hint_masked": hint_masked}
